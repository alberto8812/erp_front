...
// Función auxiliar para obtener el issuer correcto según el contexto
function getKeycloakIssuer() {
    // En el servidor (SSR/API routes), usar la URL interna
    if (typeof window === 'undefined') {
        return process.env.KEYCLOAK_ISSUER_INTERNAL || process.env.KEYCLOAK_ISSUER!;
    }
    // En el cliente, usar la URL pública
...
async function refreshAccessToken(token: Record<string, unknown>) {
    try {
        ...
        console.log(`[RefreshToken] Using issuer: ${issuer}`);
        ...
        if (!response.ok) {
            console.error(`[RefreshToken] Failed:`, refreshed);
        ...
        console.error("[RefreshToken] Error refreshing access token:", error);
    ...
    callbacks: {
        async jwt({ token, account }) {
            ...
            if (account) {
                ...
                console.log(`[JWT] Initial login, token expires at: ${new Date((account.expires_at || 0) * 1000).toISOString()}`);
                return token;
            }
