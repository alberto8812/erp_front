...
import { columnsProductCategories } from "./columns-product-category";
import { productCategoryFormConfig } from "../forms/product-category-form.config";
import type { ProductCategory } from "../../domain/entities/product-category.entity";

export function ProductCategoriesTablePage() {
  const {
    data,
    isLoading,
    pagination,
  ...
  const handleSubmit = (formData: Record<string, unknown>) => {
    if (editingItem) {
      updateMutation.mutate(
        { id: editingItem.category_id, data: formData },
  ...
  return (
    <>
      ...
      <Show when={!isLoading} fallback={<TableSkeleton columns={columnsProductCategories.length} />}>
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        title={editingItem ? "Editar Categoría" : "Crear Categoría"}
        formConfig={productCategoryFormConfig}
