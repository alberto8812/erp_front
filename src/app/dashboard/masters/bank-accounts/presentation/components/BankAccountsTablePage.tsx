...
import { columnsBankAccounts } from "./columns-bank-account";
import { bankAccountFormConfig } from "../forms/bank-account-form.config";
...
export function BankAccountsTablePage() {
  const {
    data,
    isLoading,
    pagination,
  ...
  return (
    <>
      <PageHeader pageHeader={bankAccountsHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsBankAccounts.length} />}
      >
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        title={editingItem ? "Editar Cuenta Bancaria" : "Crear Cuenta Bancaria"}
        formConfig={bankAccountFormConfig}
        defaultValues={
          editingItem
      ...
      <ExcelImportDialog
        ...
        title="Importar cuentas bancarias desde Excel"
        onSuccess={() => {}}
      />
    </>
  );
