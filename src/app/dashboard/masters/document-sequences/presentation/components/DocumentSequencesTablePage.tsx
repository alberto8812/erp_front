...
import { useDocumentSequences } from "../hooks/use-document-sequences";
import { columnsDocumentSequences } from "./columns-document-sequence";
import { documentSequenceFormConfig } from "../forms/document-sequence-form.config";
...
export function DocumentSequencesTablePage() {
  const {
    data,
    isLoading,
    pagination,
  ...
  const [editingItem, setEditingItem] = useState<DocumentSequence | null>(null);
  const documentSequencesHeader = {
    filters: [
      {
        title: "Filtros",
  ...
  const columnsWithActions = [
    ...columnsDocumentSequences,
    {
      id: "actions",
      header: "Acciones",
      cell: ({ row }: { row: { original: DocumentSequence } }) => (
  ...
  return (
    <>
      <PageHeader pageHeader={documentSequencesHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsDocumentSequences.length} />}
      >
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        title={editingItem ? "Editar Secuencia" : "Crear Secuencia"}
        formConfig={documentSequenceFormConfig}
        defaultValues={
          editingItem
      ...
      <ExcelImportDialog
        ...
        moduleKey="document_sequences"
        title="Importar  Secuencias desde Excel"
        onSuccess={() => {}}
      />
    </>
  );
