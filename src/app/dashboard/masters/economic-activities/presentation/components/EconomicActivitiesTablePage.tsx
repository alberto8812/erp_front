...
import { useEconomicActivities } from "../hooks/use-economic-activitys";
import { columnsEconomicActivities } from "./columns-economic-activity";
import { economicActivityFormConfig } from "../forms/economic-activity-form.config";
...
export function EconomicActivitiesTablePage() {
  const {
    data,
    isLoading,
    pagination,
    ...
    updateMutation,
    deleteMutation,
  } = useEconomicActivities();
  const [dialogOpen, setDialogOpen] = useState({
    dialogOpen: false,
    importOpen: false,
  });
  const [editingItem, setEditingItem] = useState<EconomicActivity | null>(null);
  const economicActivitiesHeader = {
    filters: [
      {
        title: "Filtros",
  ...
  const columnsWithActions = [
    ...columnsEconomicActivities,
    {
      id: "actions",
      header: "Acciones",
      cell: ({ row }: { row: { original: EconomicActivity } }) => (
  ...
  return (
    <>
      <PageHeader pageHeader={economicActivitiesHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsEconomicActivities.length} />}
      >
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        formConfig={economicActivityFormConfig}
        defaultValues={
          editingItem
      ...
      <ExcelImportDialog
        ...
        moduleKey="economic_activities"
        title="Importar  Actividades Económicas desde Excel"
        onSuccess={() => {}}
      />
    </>
  );
